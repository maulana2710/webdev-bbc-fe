...
React.useEffect(() => {
    ...
    return(
        <div id="default-carousel" className="relative w-screen h-screen bg-black" data-carousel="slide">
             {/* <!-- Carousel wrapper --> */}
             <div className="relative h-56 overflow-hidden rounded-lg md:h-96">
                {slides.map((image, index) => (
                    <div key={index} className={`hidden duration-700 ease-in-out ${index === currentIndex ? 'block' : 'hidden'}`} data-carousel-item>
                        <img src={image} className="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt={`img ${index + 1}`} />
                    </div>
                ))}
            </div>
            {/* <!-- Slider indicators --> */}
